...
| G2 | Skin axis (all 21 skins) as CSS custom properties consumed by Tailwind utilities | `style.css` | `theme/tokens.css` (carried forward verbatim), `theme/tailwind.css` `@theme` mapping | all | pw screenshot (default skin), manual | pass | |
...
| G8 | Responsive: desktop rail + sidebar, narrow, mobile drawer and composer config sheet, safe areas, touch targets | `style.css`, `boot.js` | Tailwind utilities on tokens, `shell/*`, `MobileNav.tsx` | all | pw screenshots at 1280x800 and 390x844 | pass | |
