...
| G2 | Skin axis (all 21 skins) as CSS custom properties consumed by Tailwind utilities | `style.css` | `theme/tokens.css` (generated from the legacy stylesheet by `scripts/css-convert.mjs`), `theme/tailwind.css` `@theme` mapping | all | pw screenshot (default skin), manual | pass | Swatch picker reproduced in Settings, Appearance. |
