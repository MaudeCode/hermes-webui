...
def _command_owner_helpers(source):
    return _js_block(
        source,
        "function createCommandOwnerContext()",
        "\nfunction commandSessionChanged()",
    )
...
def test_yolo_command_resumes_visible_approval_through_same_atomic_action():
    ...
    owner_helpers = _command_owner_helpers(commands_js)
    script = "\n".join([
        "const calls=[];",
        "const S={session:{session_id:'browser-session'}};",
        "let _loadSessionGeneration=1;",
...
@pytest.mark.skipif(not APPROVAL_AVAILABLE, reason="tools.approval unavailable")
def test_auto_approved_run_retires_preexisting_same_run_mirror(monkeypatch):
    from api import route_approvals

    sid = "webui-yolo-existing-mirror"
    run_id = "run-existing-mirror"
    approval = {
        "command": "true",
    ...
    monkeypatch.setattr(gateway_chat, "_auto_approve_gateway_run", lambda *_args: None)

    try:
        settled = gateway_chat._settle_gateway_run_approval(
            sid,
            {**approval, "approval_id": "approval-new"},
            "http://gateway",
            "",
        )
        assert settled == (True, None, 0)
        assert route_approvals.gateway_pending_mirror(sid, run_id=run_id) is None
    finally:
        disable_session_yolo(sid)
        route_approvals.retire_gateway_pending_mirror(sid, run_id=run_id)
...
@pytest.mark.skipif(not APPROVAL_AVAILABLE, reason="tools.approval unavailable")
def test_terminal_run_retirement_wakes_removed_local_waiter():
    from api import route_approvals
    from tools.approval import _ApprovalEntry

    sid = "webui-terminal-run-waiter"
    run_id = "run-terminal-waiter"
    ...
    with route_approvals._lock:
        route_approvals._gateway_queues[sid] = [entry]
    route_approvals.submit_gateway_pending_mirror(sid, dict(entry.data))

    try:
        assert route_approvals.retire_gateway_pending_mirror(sid, run_id=run_id) is True
        assert entry.event.is_set()
        assert entry.result == "deny"
        assert "run ended" in str(entry.reason).lower()
    finally:
        with route_approvals._lock:
...
def test_expired_no_run_mirror_is_retired_when_producer_disappears():
    from api import route_approvals

    sid = "webui-expired-no-run-mirror"
    mirror = {
        "approval_id": "approval-expired",
        "run_id": "",
        "_gateway_mirror": True,
        "_gateway_mirror_token": "token-expired",
    }
    with route_approvals._lock:
        route_approvals._pending[sid] = [mirror]
        route_approvals._gateway_queues.pop(sid, None)

    try:
        handled, resolved, head, total = (
            route_approvals.resolve_gateway_pending_local_no_run_mirror(
                sid, "approval-expired", "once"
            )
        )
        assert (handled, resolved, head, total) == (True, 0, None, 0)
        assert route_approvals.gateway_pending_mirror(sid) is None
    finally:
        with route_approvals._lock:
            route_approvals._pending.pop(sid, None)
