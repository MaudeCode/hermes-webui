...
def test_hermes_ext_configure_registration_invocation_and_quarantine():
    script = textwrap.dedent(
        ...
        (async () => {{
          ...
          const lateFailureReports = [];
          const diagnosticsBeforeLateFailure = diagnostics.length;
          const unregisterLateFailure = alpha.settings.registerConfigure(() => ({{
            then(resolve,reject) {{
              resolve();
          ...
          assert.strictEqual(runtime._invokeConfigure('alpha.ext', {{
            opener: connectedOpener,
            onError: error => lateFailureReports.push(error.message),
          }}), true);
          assert.deepStrictEqual(lateFailureReports, []);
          assert.strictEqual(diagnostics.length, diagnosticsBeforeLateFailure);
          assert.strictEqual(runtime._configureStateForExtension('alpha.ext').pending, false);
          assert.strictEqual(unregisterLateFailure(), true);
          ...
          assert.strictEqual(runtime._invokeConfigure('alpha.ext', {{opener: cssHiddenOpener}}), true);
          assert.strictEqual(cssHiddenOpener.focusCount, 0, 'CSS-hidden opener is not focusable');
          assert.strictEqual(replacementButton.focusCount, 2, 'focus falls back to a rendered Configure button');
          ...
          assert.strictEqual(runtime._markConfigureStatusUnavailable(), true);
          assert.deepStrictEqual(runtime._configureStateForExtension('beta.ext'), {{available: false, pending: false}});
          assert.strictEqual(runtime._invokeConfigure('beta.ext', {{opener: connectedOpener}}), false);
          assert.strictEqual(runtime._markConfigureStatusUnavailable(), false);
          runtime.primeFromStatus({{extensions: [
            {{id: 'alpha.ext', name: 'Alpha', effective_enabled: false}},
            {{id: 'beta.ext', name: 'Beta', effective_enabled: true}}
          ]}});
          assert.deepStrictEqual(runtime._configureStateForExtension('beta.ext'), {{available: true, pending: false}});

          assert.strictEqual(runtime._quarantineConfigureExtension('alpha.ext'), true);
          assert.strictEqual(runtime._quarantineConfigureExtension('alpha.ext'), false);
          assert.deepStrictEqual(runtime._configureStateForExtension('alpha.ext'), {{available: false, pending: false}});
