...
def test_pin_action_updates_local_cache_and_renders_before_refetch():
    ...
    snippet = SESSIONS_JS[api_idx:api_idx + 1400]

    cached_idx = snippet.find("for(const cached of (_allSessions||[])){")
    update_idx = snippet.find("if(cached&&targetIds.includes(cached.session_id)) cached.pinned=newPinned;")
    active_idx = snippet.find("if(S.session&&targetIds.includes(S.session.session_id)) S.session.pinned=newPinned;")
    cache_render_idx = snippet.find("renderSessionListFromCache();")
    refetch_idx = snippet.find("void renderSessionList();")

    assert cached_idx != -1, "pin action must visit cached lineage rows"
    assert update_idx != -1, "pin action must update cached lineage row pin state"
    assert active_idx != -1, "pin action must update the active lineage segment"
