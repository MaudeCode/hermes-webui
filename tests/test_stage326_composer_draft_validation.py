...
def test_draft_save_does_not_touch_session_updated_at():
    ...
    assert shell_save_idx < persist_idx
...
def test_draft_save_skips_unchanged_payload_before_persist():
    ...
    assert save_idx != -1, "draft route should still save changed drafts to its sidecar"
    assert unchanged_idx < save_idx, "unchanged guard must run before sidecar persistence"
