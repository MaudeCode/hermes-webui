...
def test_chat_start_survives_slow_provider_probe(monkeypatch):
    ...
    try:
        ...
        # directly. Keep a generous wall-clock backstop as well so a regression
        # that waits nearly the probe's five-second deadline cannot pass.
        assert rebuild_worker.is_alive()
        assert elapsed < 2.0, (
            f"get_available_models() blocked {elapsed:.2f}s despite the "
            f"{cfg._LIVE_REBUILD_BUDGET_SECONDS}s foreground budget"
        )
