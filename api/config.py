    ...
    *,
    ...
    if parsed_provider_hint is not None:
        ...
        return _finalize(
            bare_model,
            provider_hint,
            _get_provider_base_url(provider_hint, config_data=active_cfg),
        )
...
def resolve_custom_provider_connection(
    ...
    *,
    ...
    # Resolve endpoint and credential metadata from one caller-owned snapshot.
    # Profile-scoped call sites pass the session profile's config explicitly so
    # a concurrent profile switch cannot pair another endpoint with this key.
    cfg_data = (
        copy.deepcopy(config_data)
        if isinstance(config_data, dict)
        else get_config_snapshot()
...
def model_with_provider_context(
    ...
    *,
    ...
    # that endpoint hint even for HuggingFace-style model IDs containing '/';
    # otherwise the generic slash passthrough below drops the session's provider
    # and overlapping custom endpoints route through whichever provider is active.
    if provider.startswith("custom:"):
        return f"@{provider}:{model}"
