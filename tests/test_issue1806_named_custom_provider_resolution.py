...
def _setup_production_composed_runtime(
    ...
    fail_first=None,
    heal_mutate=None,
    ambient_cfg_after_snapshot=None,
):
    ...
    * ``"raised"`` -- the first ``run_conversation`` RAISES it
    ...
    ``ambient_cfg_after_snapshot`` installs a conflicting process-global config
    after the profile snapshot is frozen, proving the streaming path carries the
    selected profile's snapshot through model and connection resolution.
    """
    ...
    profile_config_snapshot = copy.deepcopy(cfg_dict)
    try:
        config._cfg_mtime = config.Path(config._get_config_path()).stat().st_mtime
    except Exception:
        config._cfg_mtime = 0.0
    config._cfg_path = config._get_config_path()
    if ambient_cfg_after_snapshot is not None:
        config.cfg.clear()
        config.cfg.update(copy.deepcopy(ambient_cfg_after_snapshot))
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: copy.deepcopy(profile_config_snapshot),
    )
    # Streaming blocks process-env fallback once a profile scope is installed.
    # Seed only credential names declared by this profile so these tests exercise
    # the same profile-local config/secret snapshot as production.
    import api.profiles as profiles

    profile_secret_env = {}
    records = list(profile_config_snapshot.get("custom_providers", []))
    records.extend(profile_config_snapshot.get("providers", {}).values())
    for record in records:
        if not isinstance(record, dict):
            continue
        env_names = [record.get("key_env"), record.get("api_key_env")]
        raw_api_key = record.get("api_key")
        if (
            isinstance(raw_api_key, str)
            and raw_api_key.startswith("${")
            and raw_api_key.endswith("}")
        ):
            env_names.append(raw_api_key[2:-1])
        for env_name in env_names:
            if isinstance(env_name, str) and env_name in os.environ:
                profile_secret_env[env_name] = os.environ[env_name]
    monkeypatch.setattr(
        profiles,
        "_profile_secret_thread_env",
        lambda *_args, **_kwargs: dict(profile_secret_env),
    )
...
def test_v12_raw_provider_record_owns_its_named_slug(monkeypatch, record, label):
    ...
    profile_snapshot = _v12_raw_cfg(record)
    # Keep a conflicting process-global config installed. The fork's explicit
    # snapshot API must resolve only the selected profile's raw v12 record.
    _with_direct_config(monkeypatch, copy.deepcopy(_SOLE_UNRELATED_ROW_CFG))

    connection = config.resolve_custom_provider_connection(
        "custom:omni", config_data=profile_snapshot
    )
    api_key, base_url = connection[0], connection[1]
    assert base_url == _V12_URL, label
    assert api_key == _V12_KEY, label

    bundle = config.resolve_custom_provider_bundle(
        "custom:omni", config_data=profile_snapshot
    )
...
def test_v12_raw_record_initial_streaming_send_builds_the_agent(monkeypatch, record, label):
    ...
    stream_id, q, captured, restore = _setup_production_composed_runtime(
        ...
        dict(_AMBIENT_SIDE_FIELD_RUNTIME),
        session_id=session_id,
        ambient_cfg_after_snapshot=copy.deepcopy(_SOLE_UNRELATED_ROW_CFG),
    )
    try:
        streaming.STREAMS[stream_id] = q
        streaming._run_agent_streaming(
