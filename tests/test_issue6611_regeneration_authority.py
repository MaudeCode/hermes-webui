...
def test_all_terminal_payloads_carry_fresh_revision():
    session = _session()
    payload = _session_payload_with_terminal_window(session)
...
def test_writable_imported_session_accepts_only_a_marked_final_user_turn(monkeypatch, tmp_path):
    ...
    assert _session_payload_with_terminal_window(session)["regeneration_revision"] == revision
...
def test_get_and_terminal_consumers_omit_imported_unowned_revision(monkeypatch):
    ...
    assert "regeneration_revision" not in _session_payload_with_terminal_window(session)
...
def test_regeneration_sidecar_path_avoids_full_transcript_refetch(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: {
            ...
            "prefix_digest": _visible_keys_digest(
                _session_message_visible_key(r) for r in prefix_rows
            ),
...
def _visible_keys_digest(keys):
    from api.models import _session_message_visible_keys_digest

    result = _session_message_visible_keys_digest(keys)
...
def _snap(prefix_count=0, prefix_keys=None, tail=None, tail_keys=None):
    ...
    return {
        ...
        "prefix_digest": _visible_keys_digest(prefix_keys or []),
        "tail": tail,
...
def test_regeneration_tail_snapshot_reads_real_sqlite(monkeypatch, tmp_path):
    ...
    prefix_keys = [
        models._session_message_visible_key({"role": role, "content": content})
        for _sid, role, content, ts in rows
        if ts < 250.0
    ]
    assert snap["prefix_digest"] == _visible_keys_digest(prefix_keys)
    ...
    assert prefix_keys[0] == snap["tail_keys"][0]


def test_regeneration_snapshot_missing_named_profile_does_not_read_active_profile(
    monkeypatch,
    tmp_path,
):
    from api import models

    active_db = _make_state_db(
        tmp_path,
        [{"role": "user", "content": "active-profile-secret", "timestamp": 100.0}],
    )
    missing_profile_home = tmp_path / "missing-profile"
    monkeypatch.setattr(models, "_active_state_db_path", lambda: active_db)
    monkeypatch.setattr(models, "_get_profile_home", lambda _profile: missing_profile_home)

    snap = models.get_state_db_regeneration_tail_snapshot(
        "s1",
        50.0,
        profile="missing",
    )
