...
| G2 | Skin axis (all 21 skins) as CSS custom properties consumed by Tailwind utilities | `style.css` | `theme/legacy.css` (the legacy stylesheet carried forward verbatim), `theme/tailwind.css` `@theme` mapping | all | pw screenshot (default skin), manual | pass | Swatch picker reproduced in Settings, Appearance. |
...
| G8 | Responsive: desktop rail + sidebar, narrow, mobile drawer, bottom tab bar and composer config sheet, safe areas, touch targets | `style.css`, `boot.js`, `hub.js` tab bar | `theme/legacy.css`, `shell/*`, `MobileNav.tsx`, `Tabbar.tsx` | all | pw screenshots at 1280x800 and 390x844 | pass | |
