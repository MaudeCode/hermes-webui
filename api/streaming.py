...
# Global lock for the remaining bounded os.environ write helpers. Agent turns
# and detached profile work use context-local state and never hold this lock.
...
def _resolve_custom_provider_connection_authority(
    ...
    profile_scope_already_bound: bool = False,
    ...
    resolved. The caller can supply the session's exact config snapshot, and this
    helper keeps credential expansion inside the matching profile scope. On a
    detached worker thread (which doesn't inherit the request-profile TLS/env),
    an unbound call would read the DEFAULT profile and pair a named profile's
    endpoint with the default profile's API key (finding #3). No-op for the
    default/root profile; when ``profile_name`` is None the ambient scope (if any)
    is used. Callers already inside that scope set
    ``profile_scope_already_bound=True`` to avoid nesting profile scopes.
    ...
    profile_scope = (
        ...
        if profile_scope_already_bound
        else _profiles_api.profile_scope_for_detached_worker(
            profile_name,
...
def _resolve_custom_provider_runtime_overrides(
    ...
    return _resolve_custom_provider_connection_authority(
        ...
        config_data=config_data,
        profile_scope_already_bound=profile_scope_already_bound,
    )[:3]
...
def _resolve_runtime_connection_bundle(
    ...
    :data:`_RUNTIME_BUNDLE_FIELDS` and the custom-route verdict/provenance fields.
    Callers must apply the whole dict — that is the point: the endpoint/credential
    and the transport/protocol/pool fields are one authority, and the agent-cache
    signature must be derived from the same dict so a bundle change always mints
    a new agent.
    """
    lookup_provider = custom_provider_lookup or resolved_provider
    from api import profiles as _profiles_api
...
def _annotate_media_snapshots_for_settled_messages(messages, *, session=None) -> None:
    ...
    try:
        if session is not None:
            from api.workspace import session_workspace_supports_local_io
...
def _persist_cancelled_turn(
    ...
    *,
    ...
    Cancel usually writes this marker before the worker finishes unwinding. A
    late worker must never clear or materialize a newer turn that now owns the
    same session.
    """
    active_stream_id = str(getattr(session, 'active_stream_id', None) or '')
    if stream_id and active_stream_id and active_stream_id != str(stream_id):
        logger.info(
            "Skipping stale cancelled-turn finalization for stream %s; active_stream_id=%s",
            stream_id,
            active_stream_id,
        )
        return False
    # cancel_stream() normally persisted the terminal marker already. Check this
    # BEFORE touching pending state so a delayed worker cannot consume a successor
    # turn merely to discover that its own marker was already present.
    if _session_has_cancel_marker(session):
        return False
...
def _finalize_cancelled_turn(
    ...
    *,
    ...
    """Finalize a cancelled turn only while this worker still owns writeback.

    The writeback-owner generation survives cancel cleanup and is replaced by a
    successor admission. Resolve the current session object and require exact
    stream ownership before changing or saving persistent state.
    """
    if stream_id:
        session_id = getattr(session, "session_id", None)
        ...
        if owner != stream_id:
            ...
            return False
        current_stream = getattr(current, "active_stream_id", None)
...
def _set_turn_session_identity(session_id: str, workspace: str = ""):
    ...
    Binds six context-locals so every session-key / UI-owner consumer is
...
def _run_background_title_update(session_id: str, user_text: str, assistant_text: str, placeholder_title: str, put_event, agent=None):
    ...
    try:
        ...
        with profiles_api.profile_env_for_background_worker(
            s, "background title", logger_override=logger
        ):
...
def _is_stable_message_id(value) -> bool:
    ...
    return type(value) is int and 0 < value <= _MAX_SAFE_MESSAGE_ID
...
def _run_agent_streaming(
    ...
    *,
    ...
    try:
        ...
        # First-time module initialisation can be slow, so keep it outside every
        # process-wide lock (#2024).
        ensure_agent_runtime_current()
        ...
        if _HERMES_FOUND and _streaming_requires_process_env_fallback(
            ...
            terminal_context_installed=_streaming_terminal_context_installed,
            ...
            raise RuntimeError(
                ...
                "requires process-global profile state. Upgrade Hermes Agent "
                "to the version paired with this WebUI."
            )
        _streaming_profile_env_scope_token = _STREAMING_PROFILE_ENV_SCOPE_HELD.set(False)
        # ── MCP Server Discovery (lazy import, idempotent) ──
        # MUST run after the context-local Hermes-home override above so
        # `discover_mcp_tools()` reads this session's profile config (#1968).
        #
        ...
        try:
            ...
            with profiles_api.profile_scope_for_detached_worker(
                ...
                _connection_cfg = _get_config_for_home(_profile_home)
                warm_models_catalog_provenance_if_cold()
                resolved_model, resolved_provider, resolved_base_url = resolve_model_provider(
                    model_with_provider_context(
                        model,
                        provider_context,
                        config_data=_connection_cfg,
                    ),
                ...
                _runtime_bundle = _resolve_runtime_connection_bundle(
                    ...
                    config_data=_connection_cfg,
                    profile_scope_already_bound=True,
                )
            ...
            if s is not None:
                ...
                with _terminal_writeback_lock("exception_cancel") as acquired:
                    ...
                    if (
                        ...
                        _pause_recorded = _merge_process_wakeup_pause_into_current_session(
                            s,
                            classification=_classification['type'],
                            model=_turn_route_model,
                            provider=_turn_route_provider,
                        )
                    _cancel_finalized = _finalize_cancelled_turn(
                        s,
                        ephemeral=ephemeral,
                        stream_id=stream_id,
                    )
