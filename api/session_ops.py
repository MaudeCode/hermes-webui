...
def apply_regeneration_plan(
    ...
    *,
    ...
    prepared_context, retained_context_user_index = truncate_context_for_display_keep(
        context,
        rows,
    ...
    if retained_context_user_index is not None and retained_context_user_index < len(session.context_messages):
        context_row = session.context_messages[retained_context_user_index]
...
def _bounded_tail_snapshot_if_safe(session, read_floor):
    ...
    from api.models import (
        ...
        _session_message_visible_keys_digest,
    ...
    tail_key_set = set(snap["tail_keys"])
    prefix_tail_collision = False

    def _sidecar_prefix_keys():
        nonlocal prefix_tail_collision
        for message in getattr(session, "messages", None) or []:
            if not isinstance(message, dict):
                continue
            ts = _message_timestamp_as_float(message)
            if ts is not None and ts < read_floor:
                key = _session_message_visible_key(message)
                if key is None:
                    return
                if key in tail_key_set:
                    prefix_tail_collision = True
                yield key

    sidecar_digest_result = _session_message_visible_keys_digest(
        _sidecar_prefix_keys()
    )
    if sidecar_digest_result is None:
        return None
    sidecar_count, sidecar_digest = sidecar_digest_result
    if sidecar_count != prefix.get("count") or sidecar_digest != snap.get("prefix_digest"):
...
def truncate_context_for_display_keep(
    ...
    *,
    ...
    When ``return_boundary_index`` is true, the second value is the exact
    context row aligned to ``full_messages[keep - 1]``. It is ``None`` when the
    retained display boundary cannot be mapped without ambiguity.
    """
    ...
    if len(msgs) == 0:
        return _result([])
    # Display and model context can diverge even when their lengths match, so
    # every non-empty pair goes through identity-aware alignment. Raw slicing
    # here could relabel an unrelated context user as the retained regeneration
    # turn.
    ...
    if keep < len(msgs):
        ...
        if first_unkept is not None:
            ...
            return _result(ctx[:first_unkept])
        if last_kept is not None:
            ...
            if (
                ...
                return _result(ctx[:ambiguous_first_unkept])
            return _result(ctx[:last_kept + 1], last_kept)
