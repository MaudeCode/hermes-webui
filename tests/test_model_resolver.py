...
def test_session_provider_context_keeps_named_custom_provider_for_slash_model():
    """A slash in a custom model id must not discard the session's endpoint."""
    custom_providers = [
        {
            'name': 'dogapi',
            'base_url': 'https://www.dogapi.cc/v1',
            'models': ['vendor/shared-model'],
        },
        {
            'name': 'packyapi',
            'base_url': 'https://www.packyapi.ai/v1',
    ...
    old_cfg = dict(config.cfg)
    config.cfg['model'] = {
        'default': 'vendor/shared-model',
        'provider': 'custom:dogapi',
        'base_url': 'https://www.dogapi.cc/v1',
    }
    config.cfg['custom_providers'] = custom_providers
    try:
...
def test_model_and_custom_connection_use_one_explicit_profile_config_snapshot():
    """Provider routing and credentials must never consult the global profile."""
    profile_cfg = {
        'model': {
            'default': 'shared-model',
            'provider': 'custom:profile-b',
    ...
    config.cfg.update({
        'model': {
            'default': 'shared-model',
            'provider': 'custom:global-a',
            'base_url': 'https://global-a.example/v1',
        },
        'custom_providers': [
            {
                'name': 'global-a',
                'base_url': 'https://global-a.example/v1',
                'api_key': 'global-a-key',
                'models': ['shared-model'],
            },
        ],
    })
    try:
        ...
        api_key, connection_base_url = config.resolve_custom_provider_connection(
            provider,
            config_data=profile_cfg,
        )
    finally:
    ...
    assert encoded == '@custom:profile-b:shared-model'
    assert model == 'shared-model'
    assert provider == 'custom:profile-b'
    assert base_url == 'https://profile-b.example/v1'
    assert api_key == 'profile-b-key'
    assert connection_base_url == 'https://profile-b.example/v1'


def test_explicit_provider_base_url_uses_supplied_profile_snapshot():
    """An @provider route must not read another profile's global endpoint."""
    profile_cfg = {
        'model': {'default': 'shared-model', 'provider': 'profile-team'},
        'providers': {
            'profile-team': {
                'base_url': 'https://right-profile.example/v1',
                'models': ['shared-model'],
            },
        },
    }
    ...
    config.cfg.clear()
    config.cfg.update({
        'model': {'default': 'shared-model', 'provider': 'global-team'},
        'providers': {
            'profile-team': {
                'base_url': 'https://wrong-global.example/v1',
                'models': ['shared-model'],
            },
        },
    })
    ...
    assert model == 'shared-model'
    assert provider == 'profile-team'
    assert base_url == 'https://right-profile.example/v1'
...
def test_resolve_custom_provider_connection_collision_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(config, 'get_config_snapshot', lambda: {'custom_providers': custom_providers})
    with pytest.raises(config.AmbiguousCustomProviderError):
...
def test_cold_declared_model_uses_explicit_profile_snapshot_5979():
    """Cold provenance must not consult another profile's global config."""
    full_id = 'openai/gpt-5.4'
    old_cfg = dict(config.cfg)
    old_openai = config._PROVIDER_MODELS.get('openai')
    old_cache = config._available_models_cache
    old_fp = config._available_models_cache_source_fingerprint
    old_prov = config._models_cache_provenance
    openai_catalog = list(old_openai or [])
    if not any(isinstance(m, dict) and m.get('id') == 'gpt-5.4' for m in openai_catalog):
        config._PROVIDER_MODELS['openai'] = openai_catalog + [
            {'id': 'gpt-5.4', 'label': 'GPT-5.4'}
        ]
    profile_cfg = {
        'model': {
            'default': full_id,
            'provider': 'custom:profile-relay',
            'base_url': 'https://profile-relay.example/v1',
        },
        'custom_providers': [
            {
                'name': 'profile-relay',
    ...
    config.cfg.update({
        'model': {
            'default': 'other-model',
            'provider': 'custom:global-relay',
            'base_url': 'https://global-relay.example/v1',
        },
    })
    config._available_models_cache = None
    config._available_models_cache_source_fingerprint = None
    config._models_cache_provenance = (None, None)
    try:
        model, provider, base_url = config.resolve_model_provider(
            full_id,
            config_data=profile_cfg,
        ...
        if old_openai is None:
            config._PROVIDER_MODELS.pop('openai', None)
        else:
            config._PROVIDER_MODELS['openai'] = old_openai
        config._available_models_cache = old_cache
        config._available_models_cache_source_fingerprint = old_fp
        config._models_cache_provenance = old_prov

    assert model == full_id
    assert provider == 'custom:profile-relay'
    assert base_url == 'https://profile-relay.example/v1'
