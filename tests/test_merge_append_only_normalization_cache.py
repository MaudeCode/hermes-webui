...
def test_merge_append_only_caches_canonical_keys_and_preserves_identity(monkeypatch):
    ...
    # Source-aware reconciliation computes content keys for sidecar/state
    # ownership and visible keys for sidecar, normalized state, and raw state.
    # Reusing one cached key here would reintroduce the literal-prefix collision.
    assert set(dict(call_counts["content"]).values()) == {2}
    assert set(dict(call_counts["visible"]).values()) == {3}
