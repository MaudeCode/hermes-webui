...
4. **WebUI Host Isolation:**
   - A target-side workspace path is never treated as a path on the WebUI host. Local file browsing and mutation, uploads, Git and worktree controls, embedded terminals, project-context discovery, workspace suggestions, live media serving, and media snapshot capture fail closed or return an empty result for remote terminal profiles.
   - The local-I/O decision uses the session's profile when a session owns the operation. Ambient active-profile config must not authorize host-local access for a session from another profile.
   - Chat execution remains available because the configured remote terminal backend owns those target-side paths.
