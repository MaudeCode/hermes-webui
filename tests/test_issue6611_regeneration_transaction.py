...
@pytest.mark.parametrize("context_shape", ["trimmed", "equal_length_divergent"])
def test_unaligned_context_does_not_relabel_an_older_user_as_the_regenerated_turn(
    monkeypatch,
    context_shape,
):
    ...
    context_messages = [
        {"role": "user", "content": "older", "id": "u-old", "timestamp": 10.0},
        {"role": "assistant", "content": "older answer", "id": "a-old"},
    ]
    if context_shape == "equal_length_divergent":
        context_messages.extend(
            [
                {"role": "user", "content": "different", "id": "u-different", "timestamp": 15.0},
                {"role": "assistant", "content": "different answer", "id": "a-different"},
            ]
        )
    session = Session(
        session_id="unaligned-context-6611",
        messages=[
            {"role": "user", "content": "older", "id": "u-old", "timestamp": 10.0, "_source": "webui"},
            {"role": "assistant", "content": "older answer", "id": "a-old"},
            {"role": "user", "content": "latest", "id": "u-latest", "timestamp": 20.0, "_source": "webui"},
            {"role": "assistant", "content": "latest answer", "id": "a-latest"},
        ],
    ...
    result = apply_regeneration_plan(
        session,
        plan,
        return_context_user=True,
    )
    assert isinstance(result, tuple)
    applied, retained_context_user = result

    assert applied
    assert retained_context_user is None
    assert session.context_messages[0] == {
        "role": "user",
        "content": "older",
        "id": "u-old",
        "timestamp": 10.0,
    ...
    routes._prepare_chat_start_session_for_stream(
        ...
        stream_id="trimmed-context-stream",
        started_at=30.0,
    ...
    assert session.context_messages[0]["timestamp"] == 10.0
