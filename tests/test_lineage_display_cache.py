...
def test_loaded_sessions_carry_exact_sidecar_revision(lineage):
    routes, Session, child = lineage
    from api.models import _sidecar_stat_signature

    expected = _sidecar_stat_signature(routes.SESSION_DIR / f"{child.session_id}.json")
    loaded = Session.load(child.session_id)

    assert child._sidecar_loaded_signature is None
    assert loaded._sidecar_loaded_signature == expected


def test_oversized_lineage_is_not_retained(lineage, monkeypatch):
    routes, Session, child = lineage
    monkeypatch.setattr(routes, "_LINEAGE_DISPLAY_CACHE_MAX_ENTRY_BYTES", 64)
...
def test_child_replace_during_load_blocks_both_transcript_caches(lineage, monkeypatch):
    """Child rows and the signature in either cache must come from one revision."""
    routes, Session, child = lineage
    real_load = Session.load
    replaced = False
    ...
    def load_and_replace(cls, session_id):
        nonlocal replaced
        loaded = real_load(session_id)
        if session_id == child.session_id and not replaced:
            replaced = True
            replacement = real_load(session_id)
    ...
    monkeypatch.setattr(Session, "load", classmethod(load_and_replace))
    child._sidecar_loaded_signature = None
    state_signature = ("target-session-revision",)
    monkeypatch.setattr(
        routes,
        "_state_db_session_signature",
        lambda *_args, **_kwargs: state_signature,
    )
    ...
    routes._limited_webui_messages_for_display_with_sidecar(
        child,
        first,
        [{"role": "tool", "content": "state", "timestamp": 3000}],
        state_db_signature=state_signature,
    )
    with routes._display_merge_cache_lock:
        assert child.session_id not in routes._display_merge_cache

    second = routes._webui_sidecar_lineage_messages_for_display(child)
    assert second[-1]["content"] == "child-new"
...
def test_sessions_without_lineage_cache_their_stable_sidecar(lineage, monkeypatch):
    routes, Session, child = lineage
    routes._lineage_display_cache.clear()
    solo = Session(
    ...
    assert len(out) == 1
    assert "lineage_solo" in routes._lineage_display_cache

    def fail_load(_cls, _sid):
        raise AssertionError("warm child-only cache hit reloaded the sidecar")

    monkeypatch.setattr(Session, "load", classmethod(fail_load))
    warm = routes._webui_sidecar_lineage_messages_for_display(solo)
    assert len(warm) == 1
