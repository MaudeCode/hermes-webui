...
5. Once stable, consider pruning completed journal entries older than a retention window, but only after sidecar/index recovery has no findings. **Landed (HWEB-45):** `prune_stale_turn_journals` reclaims a session's shards once none has been appended to for `HERMES_WEBUI_TURN_JOURNAL_RETENTION_DAYS` (default 14) and the session is provably settled: no nonterminal turn, no malformed line, and a live sidecar that exists and parses. The merged journal is the unit of judgement, not the shard, because a turn submitted under one pid can be completed under another. The "only after sidecar/index recovery has no findings" condition this item asks for is enforced literally: `audit_session_recovery` runs once per pass and any session it reports on is retained, as is everything if the audit itself cannot be trusted. Retention is disabled on platforms without `fcntl`, where the appender lock is a no-op. Every expired shard is emptied under the appender's own advisory lock, with an mtime recheck, so no writer in any process can lose an event; the emptied file is unlinked only once its owning pid is provably gone.
