...
def _post_new_session_with_workspace(
    ...
    *,
    ...
    monkeypatch.setattr(routes, "get_profile_default_workspace", get_fallback)
...
def test_session_new_inherited_recovery_uses_profile_default_not_global_last_workspace(
    monkeypatch, tmp_path
):
    stale = tmp_path / "deleted-workspace"
    profile_default = tmp_path / "profile-default"
    global_last = tmp_path / "global-last"
    profile_default.mkdir()
    global_last.mkdir()
    previous = SimpleNamespace(session_id="previous-session", workspace=str(stale))

    monkeypatch.setattr(
        routes,
        "get_session",
        lambda sid, metadata_only=False: previous
        if sid == previous.session_id
        else (_ for _ in ()).throw(KeyError(sid)),
    )
    monkeypatch.setattr(routes, "get_last_workspace", lambda: str(global_last))
    monkeypatch.setattr(
        routes,
        "get_profile_default_workspace",
        lambda: str(profile_default),
    )
    monkeypatch.setattr(workspace, "_home_path", lambda: tmp_path)
    monkeypatch.setattr(workspace, "load_workspaces", lambda: [])

    resolved = routes._resolve_new_session_workspace(
        {
            "workspace": str(stale),
            "workspace_inherited_from_prev_session": True,
        },
    ...
    assert resolved == str(profile_default.resolve())
