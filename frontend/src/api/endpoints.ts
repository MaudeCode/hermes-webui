...
export const listDir = (workspace: string, path = '.', showHidden = false) => get(`api/list${qs({ workspace, path, show_hidden: showHidden ? 1 : undefined })}`, DirListingSchema)
export const readFile = (workspace: string, path: string) => get(`api/file${qs({ workspace, path })}`, FileContentSchema, { timeoutMs: 60_000 })
export const rawFileUrl = (workspace: string, path: string) => `api/file/raw${qs({ workspace, path })}`
export const saveFile = (workspace: string, path: string, content: string) => post('api/file/save', { workspace, path, content }, OkSchema, { retries: 0 })
export const createFile = (workspace: string, path: string) => post('api/file/create', { workspace, path }, OkSchema, { retries: 0 })
export const createDir = (workspace: string, path: string) => post('api/file/create-dir', { workspace, path }, OkSchema, { retries: 0 })
export const deleteFile = (workspace: string, path: string) => post('api/file/delete', { workspace, path }, OkSchema, { retries: 0 })
export const renameFile = (workspace: string, path: string, new_name: string) => post('api/file/rename', { workspace, path, new_name }, OkSchema, { retries: 0 })
export const moveFile = (workspace: string, path: string, destination: string) => post('api/file/move', { workspace, path, destination }, OkSchema, { retries: 0 })
