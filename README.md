...
| `HERMES_WEBUI_TALARIA_RELAY_URL` | *(unset)* | Optional Talaria Relay HTTPS origin. The publisher is disabled unless all four Talaria settings are present. |
| `HERMES_WEBUI_TALARIA_PUBLISHER_ID` | *(unset)* | Publisher id returned by relay enrollment. |
| `HERMES_WEBUI_TALARIA_KEY_ID` | *(unset)* | Ed25519 key id returned by relay enrollment. |
| `HERMES_WEBUI_TALARIA_PRIVATE_KEY_PATH` | *(unset)* | Absolute path to the publisher's PEM private key; keep it outside the repository with mode `0600`. |
