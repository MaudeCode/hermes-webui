(function(){
  ...
  let extensionStatusAvailable=false;
  ...
  function primeFromStatus(statusPayload){
    ...
    const wasStatusAvailable=extensionStatusAvailable;
    if(!trustedSeeded){
    ...
    for(const [id,record] of configureRegistrations){
      ...
      if(!wasStatusAvailable||!before||!after||before.effective_enabled!==after.effective_enabled){
        notifyConfigureChange(id,'status');
      }
    }
  }

  function markConfigureStatusUnavailable(){
    if(!extensionStatusAvailable) return false;
    extensionStatusAvailable=false;
    for(const [id,record] of configureRegistrations){
      if(record&&record.active) notifyConfigureChange(id,'status');
    }
    return true;
  }

  function quarantineConfigureExtension(id){
    const clean=extensionId(id);
    if(!clean) return false;
    const changed=!configureQuarantinedIds.has(clean);
    configureQuarantinedIds.add(clean);
    currentExtensionStatus.delete(clean);
    notifyConfigureChange(clean,'quarantine');
    return changed;
  }
  ...
  function configureStateForExtension(id){
    ...
    const available=!!(
      clean&&extensionStatusAvailable&&record&&record.active&&!configureQuarantinedIds.has(clean)
  ...
  function focusConfigureTarget(clean,opener){
    ...
    for(const candidate of candidates){
      try{
        ...
        try{candidate.focus();}catch(_focusError){continue;}
      }
      if(typeof document!=='undefined'&&document&&'activeElement' in document&&document.activeElement!==candidate) continue;
      return true;
    }
    return false;
  }
  ...
  const api={
    ...
    _markConfigureStatusUnavailable:markConfigureStatusUnavailable,
    _quarantineConfigureExtension:quarantineConfigureExtension,
