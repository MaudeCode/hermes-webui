...
class TestAuxiliaryModelsBackend:
    ...
    def test_set_hermes_default_model_persists_advanced_options(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config, "resolve_model_provider", lambda model, **_kwargs: (model, "openai", None))
    ...
    def test_set_hermes_default_model_persists_explicit_provider_override(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config, "reload_config", lambda: None)
        monkeypatch.setattr(config, "invalidate_models_cache", lambda: None)
        monkeypatch.setattr(config, "resolve_model_provider", lambda model, **_kwargs: (model, "", None))
    ...
    def test_set_hermes_default_model_provider_override_replaces_stale_custom_base_url(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config, "reload_config", lambda: None)
        monkeypatch.setattr(config, "invalidate_models_cache", lambda: None)
        monkeypatch.setattr(config, "resolve_model_provider", lambda model, **_kwargs: (model, "custom", "http://old.local/v1"))
    ...
    def test_set_auxiliary_model_explicit_advanced_base_url_wins_over_custom_resolution(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            ...
            lambda model, **_kwargs: (model, "custom:demo", "https://resolved.invalid/v1"),
        )
    ...
    def test_aux_slot_missing_named_provider_never_borrows_active_endpoint(
        self, monkeypatch, tmp_path
    ):
        """A stale named provider must not inherit another provider's URL."""
        import yaml
        ...
        monkeypatch.setattr(
            config,
            "resolve_model_provider",
            lambda model, **_kwargs: resolver_calls.append(model)
            or (model, "custom:b", "https://b.example/v1"),
        )

        config.set_auxiliary_model("vision", "custom:a", "shared-model")

        saved = config._load_yaml_config_file(config_path)["auxiliary"]["vision"]
        assert saved["provider"] == "custom:a"
