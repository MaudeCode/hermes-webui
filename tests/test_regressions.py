...
def test_queue_card_cross_session_clear_called_before_draft_save(cleanup_test_sessions):
    ...
    cross_body = load_body[cross_start:cross_end]
    clear_idx = cross_body.find("_clearQueueCardDisplay(currentSid);")
    save_idx = cross_body.find("await _saveComposerDraftNow(currentSid")
    assert clear_idx != -1 and save_idx != -1 and clear_idx < save_idx, (
...
def test_live_stream_tokens_persist_partial_assistant_for_session_switch(cleanup_test_sessions):
    ...
    # where split is derived from assistantText through the stateless or stateful
    # streaming think parser. The invariant — partial assistant text is mirrored
    # into INFLIGHT state — is unchanged.
    ...
    assert (
        "_splitThinkFromContent(assistantText" in messages_src
        or "_parseInflightThinkingStream(assistantText" in messages_src
    ), \
