...
# Cold followers have no safe stale value to return. Give the single owner a
# full second before falling back so scheduler jitter cannot duplicate an
# expensive projection; stale-cache callers still use the 100 ms path below.
...
def _write_session_index(updates=None, *, session_dir: Path | None = None, session_index_file: Path | None = None):
    ...
    When *updates* is provided (Session objects whose compact entries should
    be refreshed, or already-compacted rows), this patches only those logical
    rows. Passing compacted rows lets a caller bind the row to the same session
    state it just persisted instead of whatever the session looks like by the
    time this runs. The
            ...
            with LOCK:
                ...
                for update in updates:
                    if isinstance(update, dict):
                        ...
                        if sid:
                            updated_map[sid] = update
                    else:
                        updated_map[update.session_id] = update.compact()
...
def _replay_run_journal_windows(
    ...
    *,
    ...
    windows so `stream_end` after `done` still classifies as completed). Rows
    beyond the last window wait for the next pass, which resumes at the cursor.
    """
    result = {
...
def _recover_dead_run_journal(session, stream_id: str | None) -> bool:
    ...
    try:
        ...
        if not recovered_output and not terminal_error_recovered:
            ...
            if not replay['events'] or replay['visible_output']:
...
def _session_message_visible_keys_digest(keys):
    """Hash an ordered visible-key stream without retaining its payloads."""
    digest = hashlib.sha256()
    count = 0
    for key in keys:
        if key is None:
    ...
    *,
    ...
    obtained (callers must fall back to the full read). Otherwise returns::

        {
          ...
          "prefix_digest": "sha256",              # rows < floor, db order
    ...
    if not db_path.exists():
        return {
            "prefix": {"count": 0, "null_timestamp_count": 0},
    ...
    try:
        with closing(open_state_db_readonly(db_path)) as conn:
            ...
            try:
                ...
                cur.execute("ROLLBACK")
                return None
            prefix_digest_result = _session_message_visible_keys_digest(
                _session_message_visible_key(
                    {
                        "role": r["role"],
                        "content": r["content"],
                        "tool_calls": (
                            _json_loads_if_string(r["tool_calls"])
                            if "tool_calls" in r.keys() and r["tool_calls"] is not None
                            else None
                        ),
            ...
            if prefix_digest_result is None:
                cur.execute("ROLLBACK")
                return None
            prefix_digest_count, prefix_digest = prefix_digest_result
            if prefix_digest_count != prefix["count"]:
                cur.execute("ROLLBACK")
                return None
