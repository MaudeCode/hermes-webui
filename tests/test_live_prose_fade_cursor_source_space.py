...
(0, eval)(extractConst('_TRANSPARENT_FADE_CONTAINER_TAGS'));
...
@pytest.mark.skipif(NODE is None, reason="node not on PATH")
def test_transparent_fade_descends_through_structural_containers():
    """Fade spans must land in list items or cells, never under lists or tables."""
    script = (
        _HARNESS
        + """
(0, eval)([
  extractConst('_TRANSPARENT_FADE_BLOCK_TAGS'),
  extractConst('_TRANSPARENT_FADE_CONTAINER_TAGS'),
...
const tableBody = new FakeElement('div');
tableBody.appendChild(table);

process.stdout.write(JSON.stringify({
  listTarget: _transparentFadeAppendTarget(body).tagName,
  tableTarget: _transparentFadeAppendTarget(tableBody).tagName,
}));
"""
    )
    data = _run_node_module(script)
    assert data == {"listTarget": "LI", "tableTarget": "TD"}
...
// so both transparent-fade tag sets must be evaluated in the SAME eval as the
// functions that close over them (_transparentFadeAppendTarget).
(0, eval)([
  extractConst('_TRANSPARENT_FADE_BLOCK_TAGS'),
  extractConst('_TRANSPARENT_FADE_CONTAINER_TAGS'),
  extractFunc('_transparentFadeAppendTarget'),
