...
- Set `HERMES_WEBUI_SESSION_TTL` or `session_ttl_seconds` in `settings.json` to change the TTL in seconds, from 60 seconds to one year. To use a fixed lifetime, set `HERMES_WEBUI_SESSION_SLIDING=false` or `{"webui": {"session_sliding": false}}` in `settings.json`. Sliding defaults to `true`; the environment takes precedence. Expiry is measured from the last renewal, which can precede the last successful request by up to the throttle interval.
