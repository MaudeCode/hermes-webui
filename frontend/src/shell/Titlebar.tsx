...
export function Titlebar({ title, subtitle }: { title?: string; subtitle?: string }) {
  ...
  return (
    <header className="app-titlebar relative z-20 flex h-[38px] shrink-0 items-center justify-center bg-transparent border-b-0 px-3 pt-(--app-titlebar-safe-top) pl-[max(12px,env(safe-area-inset-left,0))] pr-[max(12px,env(safe-area-inset-right,0))] box-content text-[12px] text-muted select-none [-webkit-app-region:drag] max-[901px]:justify-between max-[769px]:bg-sidebar max-[769px]:border-b max-[769px]:border-b-border max-[641px]:h-[52px]" role="banner">
      ...
      <div className="app-titlebar-inner flex items-center gap-2 min-w-0 max-w-full justify-center max-[641px]:flex-[1_1_auto]">
        ...
        <span className="app-titlebar-title text-[12px] font-semibold text-text tracking-[-.01em] whitespace-nowrap overflow-hidden text-ellipsis max-w-[60vw] max-[641px]:max-w-[72vw]" id="appTitlebarTitle">{title ?? panelLabel}</span>
