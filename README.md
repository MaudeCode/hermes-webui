...
- Session actions via `⋯` dropdown per session — pin, move to project, duplicate, delete
- Inline archive/restore button beside `⋯` on each session row
...
- Native OIDC can grant owner operations to an admin group. Set `webui_oidc.owner_claim` and `owner_values` (or `HERMES_WEBUI_OIDC_OWNER_CLAIM` / `HERMES_WEBUI_OIDC_OWNER_VALUES`); matching identities may manage updates, extensions, profiles, owner credentials, and server lifecycle. Omitting both preserves the existing behavior where every unbound OIDC session is the owner.
...
- Talaria Relay registration and user enrollment are separate. An unbound owner registers the server publisher once; afterward, a profile-bound OIDC session may enroll only its server-resolved opaque profile scope. Publisher keys remain owner-controlled, and signed Live Activity/notification snapshots are split by profile.
- Signed HMAC HttpOnly session cookie with a sliding 30-day TTL by default. Successful authenticated requests renew the server expiry and cookie together, at most once per `min(TTL / 10, 1 hour)` per session. Public paths including `/api/auth/status`, logout, and failed requests do not renew it. Logout, server-side invalidation, and OIDC policy reconciliation still revoke sessions immediately.
- Set `HERMES_WEBUI_SESSION_TTL` or `session_ttl_seconds` in `settings.json` to change the TTL in seconds, from 60 seconds to one year. To use a fixed lifetime, set `HERMES_WEBUI_SESSION_SLIDING=false` or `{"webui": {"session_sliding": false}}` in `settings.json`. Sliding defaults to `true`; the environment takes precedence. Without an environment override, an unreadable or malformed settings file disables renewal until repaired. Expiry is measured from the last renewal, which can precede the last successful request by up to the throttle interval.
...
webui_oidc:
  ...
  owner_claim: groups
  owner_values: [webui-owners]
```

Create the named Hermes profiles and any owner recovery password or passkey before enabling the map. Claim values match exactly; `sub` is the default and is preferred over mutable claims such as email. A configured map fails closed when the claim is absent, unmapped, malformed, or points to a profile that does not exist. Changing the map, deleting the profile, or recreating its directory revokes existing mapped sessions. Password and passkey logins remain unbound owner sessions; profile-bound users cannot manage owner credentials, profiles, extensions, updates, or server lifecycle. `GET /api/auth/status` reports this as `can_manage_server`, and the update banner disables Update Now, force update, and clear-lock for non-owners with an inline explanation instead of a failed request. Profile `.env` files cannot override operator authentication variables.

`owner_claim` and `owner_values` are the opt-in exception to that last rule for SSO. With both set, an OIDC identity whose validated claim matches the allowlist exactly (case-sensitive, no substring or truthiness matching, string or array of strings only) gets owner permission while keeping its profile binding: it can administer the server but still cannot read another profile's data or switch profiles. Supplying either setting activates the policy, including an explicitly blank value; setting only one of the two, or an allowlist containing anything other than non-empty strings, activates it with an empty allowlist -- nobody is elevated through SSO and the legacy behavior does not return. Startup prints a warning in that case, and an operator config that cannot be read is treated the same way rather than as "policy off".

Owner permission is server-side evidence recorded at login, never inferred from a client request or from an empty `bound_profile`. It lasts for the session lifetime, including across reloads and native app exchange, with no separate owner deadline. Removing someone from the group at the IdP takes effect at their next login, once the IdP stops issuing the claim. To revoke existing sessions, change `owner_claim`, `owner_values`, the login allowlist, the issuer, or the profile map locally; those changes invalidate sessions at the next authorization check.

An operator config that exists but cannot be read or parsed is treated as unknown rather than as "nothing configured": OIDC login is refused, OIDC owner operations are denied, authentication stays required, and startup names the failure. Repair the file to restore SSO.

`${VAR}` references inside `webui_oidc` resolve against the operator's own environment: a value a profile `.env` supplied is ignored and the reference stays literal, so a contained profile cannot name the owner group or the login allowlist indirectly.
