...
for _auth_env in _WEBUI_AUTH_ENV_NAMES:
    os.environ.pop(_auth_env, None)


@pytest.fixture(autouse=True)
def _isolate_streaming_runtime_presence(monkeypatch):
    """Keep fake-agent unit tests independent of a developer's Agent checkout.

    The production stream guard fails closed when an installed Agent cannot
    prove context-local profile isolation. Most direct streaming tests replace
    ``AIAgent`` with a narrow fake and are meant to exercise later lifecycle
    behavior. Whether this machine happens to have a real Agent checkout must
    not make those tests stop at the installation compatibility guard.
    """
    try:
        import api.streaming as streaming
    except Exception:
        return
    monkeypatch.setattr(streaming, "_HERMES_FOUND", False)
