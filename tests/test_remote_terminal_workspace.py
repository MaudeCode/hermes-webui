...
def _restore_config_module_state():
    """Keep reload_config() tests from leaking authority into later files."""
    old_cfg = copy.deepcopy(api_config.cfg)
    old_cache = copy.deepcopy(api_config._cfg_cache)
    old_state = {
        name: getattr(api_config, name)
        for name in ("_cfg_mtime", "_cfg_stat_identity", "_cfg_path", "_cfg_fingerprint")
    }
    try:
        yield
    finally:
        api_config.cfg.clear()
        api_config.cfg.update(old_cfg)
        api_config._cfg_cache.clear()
        api_config._cfg_cache.update(old_cache)
        for name, value in old_state.items():
            setattr(api_config, name, value)
        api_config.invalidate_models_cache()
...
def test_explicit_config_snapshot_avoids_ambient_config_reads(monkeypatch, tmp_path):
    snapshot = {"terminal": {"backend": "ssh", "cwd": "/srv/snapshot"}}
    last_workspace = tmp_path / "last_workspace.txt"
    last_workspace.write_text("/srv/snapshot/project", encoding="utf-8")

    def fail_ambient_read(*_args, **_kwargs):
        raise AssertionError("explicit config snapshot must be authoritative")

    monkeypatch.setattr(api_config, "get_config", fail_ambient_read)
    monkeypatch.setattr(api_config, "get_config_for_profile_home", fail_ambient_read)
    monkeypatch.setattr(
        workspace,
        "_last_workspace_file_for_profile",
        lambda profile=None: last_workspace if profile == "alice" else tmp_path / "missing.txt",
    )

    assert workspace._remote_terminal_cwd(snapshot, profile="alice") == "/srv/snapshot"
    assert workspace._remote_terminal_workspace_candidate(
        "/srv/snapshot/project", profile="alice", config_data=snapshot
    ) == Path("/srv/snapshot/project")
    assert workspace._profile_default_workspace(snapshot, profile="alice") == "/srv/snapshot"
    assert workspace.get_last_workspace(profile="alice", config_data=snapshot) == (
        "/srv/snapshot/project"
    )


def test_ambient_named_profile_does_not_use_global_last_workspace(monkeypatch, tmp_path):
    from api import profiles

    profile_workspace = tmp_path / "alice"
    profile_workspace.mkdir()
    global_workspace = tmp_path / "global"
    global_workspace.mkdir()
    global_last_workspace = tmp_path / "global-last-workspace.txt"
    global_last_workspace.write_text(str(global_workspace), encoding="utf-8")

    monkeypatch.setattr(profiles, "get_active_profile_name", lambda: "alice")
    monkeypatch.setattr(
        workspace,
        "_last_workspace_file_for_profile",
        lambda profile=None: tmp_path / "missing-profile-last-workspace.txt",
    )
    monkeypatch.setattr(workspace, "_GLOBAL_LW_FILE", global_last_workspace)

    config_snapshot = {
        "workspace": str(profile_workspace),
        "terminal": {"backend": "local"},
    }
    assert workspace.get_last_workspace(config_data=config_snapshot) == str(
...
def test_remote_terminal_workspace_paths_outside_cwd_still_reject(
    ...
    with pytest.raises(ValueError, match="outside the configured remote terminal workspace"):
        workspace.validate_workspace_to_add(str(host_only))

    with pytest.raises(ValueError, match="outside the configured remote terminal workspace"):
        workspace.resolve_trusted_workspace(str(host_only))

    cleaned = workspace._clean_workspace_list(
        [
            {"name": "Remote", "path": f"{REMOTE_CWD}/project"},
            {"name": "Host only", "path": str(host_only)},
        ]
    )
    assert [entry["name"] for entry in cleaned] == ["Remote"]
...
def test_isolated_profile_config_read_does_not_mutate_or_read_shared_cache(monkeypatch, tmp_path):
    ...
    assert workspace._remote_terminal_cwd(profile="alice") == "/srv/remote-alice"

    # 2. bob resolves to None (local backend)
    assert workspace._remote_terminal_cwd(profile="bob") is None
...
def test_remote_saved_workspace_cannot_authorize_host_only_path(monkeypatch, tmp_path):
    """Saved picker state cannot widen a remote profile onto the WebUI host."""
    from api import profiles
    ...
    # A stale or crafted saved entry outside terminal.cwd cannot authorize the
    # same path on the WebUI host, even when that local directory exists.
    with pytest.raises(ValueError, match="outside the configured remote terminal workspace"):
        workspace.resolve_trusted_workspace(str(external_dir), profile="carol")
...
class TestRound4ProfileIsolation:
    ...
    def test_cli_projection_uses_own_cli_profile(self, monkeypatch, tmp_path):
        ...
        def fake_getlw(profile=None, *, config_data=None):
...
def test_remote_workspace_git_routes_reject_before_local_repo_access(monkeypatch, tmp_path):
    """Remote workspace Git controls must never operate on a same-path local repo."""
    from urllib.parse import urlparse

    from api import routes
    from api import workspace_git

    local_collision = tmp_path / "remote-repo"
    local_collision.mkdir()
    session = type(
        "Session",
        (),
        {"session_id": "remote-git", "workspace": str(local_collision), "profile": "alice"},
    )()
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("remote Git route touched the local repository")
        ),
    )

    handler = _GitCaptureHandler()
    assert routes._handle_git_status(
        handler, urlparse("/api/git/status?session_id=remote-git")
    ) is True
...
@pytest.mark.parametrize(
    "handler_name,body",
    [
        ("_handle_git_commit_message", {"session_id": "remote-git"}),
        (
            "_handle_git_commit_message_selected",
            {"session_id": "remote-git", "paths": ["tracked.txt"]},
        ),
    ],
)
def test_remote_workspace_git_message_routes_share_local_io_gate(
    monkeypatch, tmp_path, handler_name, body
):
    """Commit-message helpers read Git state, so they must share the same gate."""
    from api import routes
    from api import workspace_git
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("remote commit-message route read the local repository")
        ),
    )
    monkeypatch.setattr(
        workspace_git,
    ...
    handler = _GitCaptureHandler()
    assert getattr(routes, handler_name)(handler, body) is True

    assert handler.status == 400
    assert handler.json_body()["code"] == "remote_workspace_unsupported"


def test_remote_workspace_file_routes_reject_same_path_local_collision(
    monkeypatch, tmp_path
):
    """The shared file-route gate blocks reads, writes, and directory listing."""
    from urllib.parse import urlparse

    from api import routes

    local_collision = tmp_path / "remote-files"
    local_collision.mkdir()
    secret = local_collision / "secret.txt"
    secret.write_text("local-only", encoding="utf-8")
    session = type(
        "Session",
        (),
        {"session_id": "remote-files", "workspace": str(local_collision), "profile": "alice"},
    )()
    monkeypatch.setattr(routes, "get_session", lambda _sid: session)
    ...
    read_handler = _GitCaptureHandler()
    assert routes._handle_file_read(
        read_handler,
        urlparse("/api/file/read?session_id=remote-files&path=secret.txt"),
    ) is True
    assert read_handler.status == 400
    assert read_handler.json_body()["error"] == "remote_workspace_unsupported"

    save_handler = _GitCaptureHandler()
    assert routes._handle_file_save(
        save_handler,
        {"session_id": "remote-files", "path": "secret.txt", "content": "overwritten"},
    ) is True
    assert save_handler.status == 400
    assert secret.read_text(encoding="utf-8") == "local-only"

    list_handler = _GitCaptureHandler()
    routes._handle_list_dir(
        list_handler,
        urlparse("/api/files?session_id=remote-files"),
    )
    assert list_handler.status == 400
    assert list_handler.json_body()["error"] == "remote_workspace_unsupported"


def test_remote_profile_terminal_start_uses_session_profile_authority(
    monkeypatch, tmp_path
):
    """A local ambient profile cannot start a host PTY for a remote session."""
    from api import routes
    from api import terminal

    local_collision = tmp_path / "remote-terminal"
    local_collision.mkdir()
    session = type(
        "Session",
        (),
        {"session_id": "remote-terminal", "workspace": str(local_collision), "profile": "alice"},
    )()
    monkeypatch.setattr(routes, "get_session", lambda _sid: session)
    ...
    monkeypatch.setattr(
        ...
        "start_terminal",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("remote session started a host-local terminal")
        ),
    )

    handler = _GitCaptureHandler()
    routes._handle_terminal_start(handler, {"session_id": "remote-terminal"})
    assert handler.status == 400
    assert handler.json_body()["error"] == "remote_terminal_backend_unsupported"


def test_remote_session_local_media_and_project_context_fail_closed(monkeypatch, tmp_path):
    """Remote MEDIA tokens and context paths never authorize same-path host files."""
    from urllib.parse import quote, urlparse

    from api import auth
    from api import routes

    local_collision = tmp_path / "remote-project"
    local_collision.mkdir()
    victim = local_collision / "report.html"
    victim.write_text("host-only", encoding="utf-8")
    session = type(
        "Session",
        (),
        {"session_id": "remote-media", "workspace": str(local_collision), "profile": "alice"},
    )()
    monkeypatch.setattr(routes, "get_session", lambda _sid: session)
    ...
    parsed = urlparse(
        f"/api/media?path={quote(str(victim))}&session_id=remote-media"
    )
    handler = _GitCaptureHandler()
    routes._handle_media(handler, parsed)
    assert handler.status == 400
    assert handler.json_body()["error"] == "remote_workspace_unsupported"

    snapshot_handler = _GitCaptureHandler()
    routes._handle_media(
        snapshot_handler,
        urlparse(
            f"/api/media?path={quote(str(victim))}&session_id=remote-media&snap={'a' * 64}"
        ),
    )
    assert snapshot_handler.status == 400
    assert snapshot_handler.json_body()["error"] == "remote_workspace_unsupported"

    monkeypatch.setattr(routes, "profile_supports_local_io", lambda _profile=None: False)
    active_profile_handler = _GitCaptureHandler()
    routes._handle_media(
        active_profile_handler,
        urlparse(f"/api/media?path={quote(str(victim))}"),
    )
    assert active_profile_handler.status == 400
    assert active_profile_handler.json_body()["error"] == "remote_workspace_unsupported"

    assert routes._memory_project_context_workspace(
        urlparse("/api/memory?session_id=remote-media")
    ) is None


def test_remote_session_settle_skips_local_media_snapshot(monkeypatch, tmp_path):
    """Settle-time snapshotting never opens host files for a remote session."""
    from api import media_snapshots
    from api import streaming

    local_collision = tmp_path / "remote-snapshot"
    local_collision.mkdir()
    victim = local_collision / "report.html"
    victim.write_text("host-only", encoding="utf-8")
    session = type(
        "Session",
        (),
        {"session_id": "remote-snapshot", "workspace": str(local_collision), "profile": "alice"},
    )()
    monkeypatch.setattr(
        api_config,
        "get_config_for_profile_home",
    ...
    monkeypatch.setattr(
        ...
        lambda _messages: called.append(True),
    )

    streaming._annotate_media_snapshots_for_settled_messages(
        [{"role": "assistant", "content": f"MEDIA:{victim}"}],
        session=session,
    )
    assert called == []


def test_remote_session_and_chat_workspace_resolution_remain_enabled(monkeypatch, tmp_path):
    """The local-I/O gate must not reject target-side session/chat execution paths."""
    ...
    session = models.Session(
        ...
        workspace="/srv/remote-alice/project",
        profile="alice",
    )
    resolved = workspace._resolve_path(session.workspace, profile=session.profile)
    message = streaming._build_native_multimodal_message(
        "",
        "keep running remotely",
        attachments=[],
        workspace=str(resolved),
        profile=session.profile,
    )

    assert session.workspace == "/srv/remote-alice/project"
    assert str(resolved) == "/srv/remote-alice/project"
    assert message == "keep running remotely"
