...
def retire_gateway_pending_mirror(
    ...
    for waiter in retired_waiters:
        ...
        event = getattr(waiter, "event", None)
    ...
    with _lock:
        ...
        for index, entry in enumerate(gateway_queue):
            ...
            if str(data.get("approval_id") or "").strip() == approval_id:
                ...
                break
        if target is None:
            # The local producer disappeared after the card was mirrored. The
            # mirror is now stale visibility state, not an actionable approval.
            # Retire it and notify subscribers instead of preserving an immortal
            # card that can never resolve.
