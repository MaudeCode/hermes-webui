...
def safe_platform_temp_root(*protected_roots: Path) -> Path | None:
    """Return the process temp root only when it is a narrow, private root."""
    try:
        ...
        if os.name == "nt":
            ...
            if not local_app_data:
                return None
            canonical = (Path(local_app_data).expanduser() / "Temp").resolve()
            if candidate != canonical:
                return None
            # A temp root must never equal or contain a protected root. The
            # canonical Windows temp directory may legitimately live below HOME.
            if any(root == candidate or root.is_relative_to(candidate) for root in protected):
                return None
            return candidate

        # POSIX temp roots sourced from TMPDIR are accepted only when they are
        # disjoint from protected trees and private to this uid. This admits the
        # macOS /private/var/folders/.../T and /run/user/<uid> shapes without
        # letting TMPDIR=$HOME widen /api/media to the user's whole home.
        if any(
            candidate == root
            ...
            with os.fdopen(source_fd, "rb", closefd=True) as src, open(tmp_path, "wb") as dst:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    copied += len(chunk)
                    if cap and copied > cap:
                        raise OSError("media snapshot exceeded per-file cap during copy")
                    digest.update(chunk)
                    dst.write(chunk)
                dst.flush()
                os.fsync(dst.fileno())
