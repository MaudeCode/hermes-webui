    ...
    def _cli_workspace():
        if _cli_workspace_cache[0] is None:
            try:
                ...
                # Compatibility for older test doubles/extensions that do not
                # yet accept the caller-owned config snapshot.
                try:
                    workspace = get_last_workspace(profile=_cli_profile)
                except TypeError:
                    workspace = get_last_workspace()
            _cli_workspace_cache[0] = str(workspace)
