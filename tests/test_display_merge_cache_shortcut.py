...
def test_writeback_owner_blocks_cache_after_persisted_flags_clear(monkeypatch, stable_key):
    session = _Session()
    sidecar = [{"role": "user", "content": "hi", "timestamp": 10.0}]
    _seed(session, sidecar, [{"role": "user", "content": "cached", "timestamp": 10.0}])

    monkeypatch.setattr(
        routes.api_config,
        "session_writeback_owner",
        lambda sid: "worker-1" if sid == session.session_id else None,
    )
    assert routes._display_merge_cached_messages(session, sidecar) is None


def test_active_run_registry_blocks_cache_without_session_flags(monkeypatch, stable_key):
    session = _Session()
    sidecar = [{"role": "user", "content": "hi", "timestamp": 10.0}]
    _seed(session, sidecar, [{"role": "user", "content": "cached", "timestamp": 10.0}])

    monkeypatch.setattr(routes.api_config, "session_writeback_owner", lambda _sid: None)
    monkeypatch.setattr(
        routes.api_config,
        "ACTIVE_RUNS",
        {"worker-1": {"session_id": session.session_id}},
    )
    assert routes._display_merge_cached_messages(session, sidecar) is None
...
class _GetHandler:
    ...
    def _safe_webui_print(self, _message):
        pass
...
def test_route_cache_hit_skips_load_and_preserves_absolute_pagination(monkeypatch, tmp_path):
    """The real GET route must keep a capped tail in the full cursor space."""
    sid = "20260822_120000_routecache"
    session = Session(
        session_id=sid,
        title="Cache route",
    ...
    rows = [
        {"role": "user", "content": f"row-{index}", "timestamp": float(index)}
        for index in range(5)
    ]
    loads = []

    def load_rows(_sid, _profile, reader_kwargs):
        loads.append(dict(reader_kwargs))
        limit = reader_kwargs.get("limit")
        selected = rows[-int(limit):] if limit else rows
        return list(selected), "SIG"
    ...
    monkeypatch.setattr(routes, "_state_db_display_row_count", lambda *_args, **_kwargs: 5)
    monkeypatch.setattr(routes, "_STATE_DB_DISPLAY_ROW_BACKSTOP", 2)
    monkeypatch.setattr(routes, "_load_state_db_messages_with_stable_signature", load_rows)
    monkeypatch.setattr(routes, "_state_db_session_signature", lambda *_args, **_kwargs: "SIG")
    monkeypatch.setattr(
        routes,
        "_display_merge_cache_key",
        lambda *_args, **_kwargs: ("sidecar", (), 0, None, "SIG", None, None),
    )
    monkeypatch.setattr(routes, "_display_merge_session_is_active", lambda _session: False)
    ...
    first = get(f"/api/session?session_id={sid}&msg_limit=1&resolve_model=0")
    assert [row["content"] for row in first["messages"]] == ["row-4"]
    assert first["message_count"] == 5
    assert first["_messages_offset"] == 4
    assert loads == [{"profile": None, "limit": 2}]

    warm = get(f"/api/session?session_id={sid}&msg_limit=1&resolve_model=0")
    assert warm["_messages_offset"] == 4
    assert len(loads) == 1, "warm route hit reloaded state.db"

    older = get(
        f"/api/session?session_id={sid}&msg_limit=1&msg_before=4&resolve_model=0"
    )
    assert [row["content"] for row in older["messages"]] == ["row-3"]
    assert older["message_count"] == 5
    assert older["_messages_offset"] == 3
    assert loads[-1] == {"profile": None}
    ...
    monkeypatch.setattr(
        routes,
        "_state_db_session_signature",
        lambda *_args, **_kwargs: "OLD-SIG",
    )
    raced = get(f"/api/session?session_id={sid}&msg_limit=1&resolve_model=0")
    assert [row["content"] for row in raced["messages"]] == ["row-4"]
    assert raced["message_count"] == 5
    assert raced["_messages_offset"] == 4
    assert loads == [
...
def test_oversized_merged_transcript_is_not_retained(monkeypatch, stable_key):
    session = _Session()
    sidecar = [{"role": "user", "content": "sidecar", "timestamp": 1.0}]
    rows = [{"role": "assistant", "content": "x" * 256, "timestamp": 2.0}]
    monkeypatch.setattr(routes, "_DISPLAY_MERGE_CACHE_MAX_ENTRY_BYTES", 64)
