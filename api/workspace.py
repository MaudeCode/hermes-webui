...
def _remote_terminal_cwd(
    config_data: dict | None = None, *, profile: str | Path | None = None
) -> str | None:
    ...
    try:
        if config_data is None:
            ...
            profile_home = _resolve_profile_home_param(profile)
            config_data = get_config_for_profile_home(profile_home)
...
REMOTE_WORKSPACE_UNSUPPORTED_CODE = "remote_workspace_unsupported"
REMOTE_WORKSPACE_UNSUPPORTED_MESSAGE = (
    "Workspace file operations are only supported for local terminal backends."
)


def profile_supports_local_io(profile=None) -> bool:
    """Return whether host-local workspace operations are safe for a profile.

    Remote workspace paths are target-side identifiers. Treat config lookup or
    malformed terminal config as unsupported so a same-named local directory
    cannot become an accidental read or write target.
    """
    try:
        from api import config as config_api

        profile_home = _resolve_profile_home_param(profile)
        config_data = config_api.get_config_for_profile_home(profile_home)
        ...
        if not isinstance(terminal_cfg, dict):
            return False
        return not _is_remote_terminal_backend(terminal_cfg)
    except Exception:
        return False


def session_workspace_supports_local_io(session=None, *, profile=None) -> bool:
    """Return whether host-local workspace operations are safe for a session."""
    if profile is None and session is not None:
        profile = getattr(session, "profile", None)
    return profile_supports_local_io(profile)


def _reject_blocked_remote_workspace_path(path: str | Path) -> None:
    """Reject blocked roots without consulting the WebUI host filesystem."""
    raw = _strip_surrounding_quotes(str(path)).strip()
    normalized = _normalize_posix_path(raw)
    candidate = Path(normalized) if normalized is not None else Path(raw)
    if _is_blocked_workspace_path(candidate, raw):
        raise ValueError(f"Path points to a system directory: {candidate}")


def _profile_default_workspace(
    config_data: dict | None = None, *, profile: str | Path | None = None
) -> str:
    ...
    try:
        ...
        for key in ('workspace', 'default_workspace'):
            ...
            if ws:
                if remote_terminal:
                    return str(ws).strip()
                p = _safe_resolve(_expanduser_path(str(ws)))
        ...
        if isinstance(terminal_cfg, dict):
            ...
            if cwd and str(cwd) not in ('.', ''):
                ...
                p = _safe_resolve(_expanduser_path(str(cwd)))
                if remote_terminal or p.is_dir():
                    return str(p)
    ...
    try:
        ...
        return str(_safe_resolve(_expanduser_path(_LIVE_DEFAULT_WORKSPACE)))
    except Exception:
        return str(_safe_resolve(_expanduser_path(_BOOT_DEFAULT_WORKSPACE)))
...
def _clean_workspace_list(workspaces: list, profile: str | Path | None = None) -> list:
    ...
    result = []
    local_io_allowed = profile_supports_local_io(profile)
    for w in workspaces:
        path = w.get('path', '')
        name = w.get('name', '')
        if not path:
...
def get_last_workspace(
    profile: str | Path | None = None, *, config_data: dict | None = None
) -> str:
    if config_data is not None:
    ...
    def valid_last_workspace(raw: str) -> str | None:
        ...
        if remote_cwd:
            ...
            if _remote_terminal_workspace_candidate(
                raw, profile=profile, config_data=config_data
            ) is not None:
                return raw
            return None
        if Path(raw).is_dir():
    ...
    if profile is None:
        try:
            ...
            _global_fallback_allowed = get_active_profile_name() in (None, '', 'default')
        except Exception:
            # Embeddings without profile support retain the historical root-state
            # fallback; named profiles are handled by the branch below.
    ...
    return _profile_default_workspace(config_data, profile=profile)
...
def list_workspace_suggestions(
    ...
    """
    if not profile_supports_local_io(profile):
        return []
...
def resolve_trusted_workspace(path: str | Path | None = None, profile: str | Path | None = None) -> Path:
    ...
    if path in (None, ""):
        if not profile_supports_local_io(profile):
            default_remote = _profile_default_workspace(profile=profile)
            remote_candidate = _remote_terminal_workspace_candidate(
                default_remote, profile=profile
            )
            if remote_candidate is None:
                raise ValueError("Remote terminal workspace is not configured")
            return remote_candidate
    ...
    if remote_candidate is not None:
        return remote_candidate
    if not profile_supports_local_io(profile):
        _reject_blocked_remote_workspace_path(path)
        raise ValueError("Path is outside the configured remote terminal workspace")
