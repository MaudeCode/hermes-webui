...
function makeFooterDom(opts) {
  ...
  const classList = {
    add() { for (const n of arguments) classes.add(n); layout('classList.add:' + Array.from(arguments).join(',')); },
    remove() { for (const n of arguments) classes.delete(n); layout('classList.remove:' + Array.from(arguments).join(',')); },
    toggle(name, force) {
...
function runFit(fit, opts) {
  ...
  const probe = dom.samples.filter(s =>
    s.reason === 'measure'
    || s.reason.includes('cf-icons')
    || s.reason.includes('cf-burger')
  );
  return {
...
eval(extractFunc('_shouldCollapseComposer'));
