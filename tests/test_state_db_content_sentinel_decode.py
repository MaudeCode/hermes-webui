...
def test_regeneration_prefix_digest_and_tail_key_agree_for_a_sentinel_row(tmp_path, monkeypatch):
    """The same row digested as prefix and keyed as tail must share one identity.
    ...
    expected = _session_message_visible_keys_digest([as_tail["tail_keys"][0]])
    assert expected is not None
    assert as_prefix["prefix"]["count"] == expected[0] == 1
    assert as_prefix["prefix_digest"] == expected[1]
