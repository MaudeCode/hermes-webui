...
def resolve_model_provider(
    ...
    *,
    ...
    def _finalize(model: object, provider: object, base_url: object) -> tuple:
        ...
        if isinstance(provider, str) and provider.startswith("custom:"):
            _unique_custom_provider_entry(
                active_cfg.get('custom_providers', []),
    ...
    if parsed_provider_hint is not None:
        ...
        #
        # Match via _unique_custom_provider_entry on the caller-owned snapshot:
        ...
        if provider_hint.startswith("custom:"):
            ...
            if entry is not None:
                ...
                base_url = _get_provider_base_url(provider_hint, config_data=active_cfg)
        else:
            base_url = _get_provider_base_url(provider_hint, config_data=active_cfg)
...
def resolve_custom_provider_connection(
    ...
    *,
    ...
    # Resolve every field from one caller-owned snapshot. Profile-scoped callers
    # pass the session profile's config so a concurrent profile switch cannot
    # pair another profile's endpoint with this credential.
    cfg_data = (
        copy.deepcopy(config_data)
        if isinstance(config_data, dict)
        else get_config_snapshot()
    )
    record, _source, is_exact, _status = _select_custom_provider_record(pid, slug, cfg_data)
    if record is None:
...
def _custom_provider_runtime_bundle_with_provenance(
    ...
    *,
    ...
    custom = resolve_custom_provider_bundle(
        lookup,
        connection_resolver=connection_resolver,
        config_data=config_data,
    )
...
def model_with_provider_context(
    ...
    *,
    ...
    # A persisted named custom provider is authoritative session context. Keep
    # that endpoint hint for non-slash IDs. Slash IDs must first prove that the
    # named provider still resolves uniquely below.
    if provider.startswith("custom:") and "/" not in model:
        return f"@{provider}:{model}"
            ...
            if provider and (provider.startswith("custom:") or provider == "custom"):
                ...
                if provider.startswith("custom:"):
                    ...
                    if not resolved_base_url:
                        # Never retain an endpoint from the slot's previous
                        # provider when the selected named provider is stale or
                        # has no configured endpoint. Advanced options below may
                        # still supply an explicit base_url.
...
STREAM_REASONING_TITLES: dict = {}  # stream_id -> normalized reasoning-title snapshot
